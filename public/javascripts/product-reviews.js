(function () {
  ...
  function loadReviews() {
    fetch(`/api/reviews/${type}/${id}`)
      ...
      .catch((err) => {
        ...
        document.getElementById('reviews-container').innerHTML = '<div class="alert alert-danger">Error loading reviews. Please refresh the page.</div>';
      });
  }
  ...
  function displayReviewSummary(data) {
    ...
    document.getElementById('total-reviews').textContent = total === 0 ? 'No reviews yet' : `Based on ${total} ${total === 1 ? 'review' : 'reviews'}`;
