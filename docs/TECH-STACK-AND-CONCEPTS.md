...
| **bcrypt-nodejs** | Latest | Password hashing | - Hashing vs encryption<br>- Salt rounds<br>- Password security |
...
| **stripe** | Latest | Payment processing | - Payment intents<br>- Webhooks<br>- PCI compliance |
...
| Technology | Version | Purpose | Concepts to Learn |
|------------|---------|---------|-------------------|
| **MongoDB** | 4.4 | Primary database | - Documents & collections<br>- BSON format<br>- Indexes<br>- Queries |
| **Mongoose** | 3.1.6 | ODM (Object Data Modeling) | - Schemas<br>- Models<br>- Validation<br>- Middleware (hooks) |
...
**Our Pipeline** (`.github/workflows/ci.yml`):
```
On Push to main:
  ├─ Job: lint          (ESLint code style)
  ├─ Job: test          (Jest unit/integration tests)
  ├─ Job: security      (npm audit vulnerabilities)
  ├─ Job: build         (Create tarball + metadata)
  ├─ Job: docker-build  (Build & save Docker image)
  ├─ Job: deploy-staging (Simulate deployment)
  └─ Job: deploy-production (Manual approval needed)
```

**Key Concepts:**

1. **Workflow**: YAML file defining automation
2. **Trigger**: What starts the workflow (push, PR, schedule)
3. **Job**: Group of steps that run on same runner
4. **Step**: Individual command or action
5. **Artifact**: Files saved between jobs (tarball, Docker image)
6. **Environment**: Deployment target (staging, production)
